...
namespace EcoSens_API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            var user = _context.Usuarios.SingleOrDefault(u => u.Correo == request.Correo && u.Contrasena == u.Contrasena);
            if (user == null)
            ...
            if(user.Tipo.Nombre_Tipo == "admin")
            {
                var empleados= _context.Empleados.ToList();
                return Ok(new
                {
                    token,
                    tipo = user.Tipo.Nombre_Tipo,
                    empleados
                });
            }

            //**Si es un empleado muestra los conjuntos de contenedores
            if (user.Tipo.Nombre_Tipo == "empleado")
            {
        ...
        private string GenerarToken(Usuario user)
        {
            ...
            var tokenDescriptor = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(new[]
                {
                    ...
                    new Claim(ClaimTypes.Role, user.Tipo.Nombre_Tipo)
                }),
